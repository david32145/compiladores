...
\section{Conjunto First e Follow}

\begin{center}
\begin{tabular}{|c|c|c|}
  \hline
  Não terminais & First & Follow \\ \hline
  program & $\{main\}$ & \\ \hline
  expr & $\{id, !, -, +, (, integer\_literal, char\_literal, float\_literal, string\_literal\}$ & \\ \hline
  expr' & $\{||, ==, !=, <=, >, >=, \&\&, +, -, *, /, \%\}$ & \\ \hline
  expr\_stmt & \makecell{$\{id, !, -, +, (, integer\_literal, char\_literal, $ \\ $float\_literal, string\_literal, \epsilon\}$} & \\ \hline
  if\_stmt & $\{if\}$ & \\ \hline
  stmt & \makecell{$\{id, !, -, +, (, integer\_literal, char\_literal, $ \\ $float\_literal, string\_literal, \epsilon\, if, \{, data\_type\}$} & \\ \hline
  decl\_block & $\{\{\}$ & \\ \hline
  var\_decl & $\{data\_type\}$ & \\ \hline
  stmt\_list & \makecell{$\{id, !, -, +, (, integer\_literal, char\_literal, $ \\ $float\_literal, string\_literal, \epsilon\, if, \{, data\_type\}$} & \\ \hline
  stmt\_list' & \makecell{$\{id, !, -, +, (, integer\_literal, char\_literal, $ \\ $float\_literal, string\_literal, \epsilon\, if, \{, data\_type\}$} & \\
  \hline
\end{tabular}
\end{center}
